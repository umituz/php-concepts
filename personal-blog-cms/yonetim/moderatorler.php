...
{
  $aranan=$_POST['aranan'];
  $moderatorsor=$db->prepare("SELECT * FROM moderator WHERE moderator_ad LIKE '%$aranan%' ORDER BY moderator_id DESC");
  $moderatorsor->execute();
  $say=$moderatorsor->rowCount();
}
else
{
  $moderatorsor=$db->prepare("SELECT * FROM moderator ORDER BY moderator_id DESC");
  $moderatorsor->execute();
  $say=$moderatorsor->rowCount();
}
?>
            ...
            <div class="x_content">
              ...
              <div class="table-responsive">
                <table class="table table-striped jambo_table bulk_action">
                  ...
                  <tbody>
                    <?php while ($moderatorcek=$moderatorsor->fetch(PDO::FETCH_ASSOC)) { extract($moderatorcek); ?>

                      <tr class="even pointer">                        
                        <td class=""><img style="width:150px; height: 75px;" src="../dosyalar/resimler/slider/<?php echo $slider_resimyol; ?>" alt=""></td>
                        <td style="font-weight: bolder; font-size: 17px;" class="text-center"><?php echo $moderator_ad; ?></td>
